...
describe('GET /api/v1/reports/customer-categories', () => {
  it('returns customer category counts as JSON numbers without period parameters', async () => {
    ...
    const response = await authenticatedRequest(createManagerAuthenticatedTestApp({ customerCategoryService }))
  ...
  it('returns HTTP 503 when the database is not configured', async () => {
    const response = await authenticatedRequest(createManagerAuthenticatedTestApp()).get('/api/v1/reports/customer-categories');
