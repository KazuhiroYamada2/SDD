...
リポジトリ名の「SDD」は「Spec-Driven Development（仕様駆動開発）」の略称です。このリモートリポジトリには、書籍で記載されている仕様駆動開発の「4 つの原則」と「7 つの工程」に沿ったサンプルファイルが含まれており、**実際のプロジェクトでどのようにリポジトリを構成するかの参考**になります。
...
- **書籍の練習**: 『仕様駆動開発 実践入門』を読みながら、「7 つの工程」のサンプルファイルを手元で確認する
- **仕様駆動開発導入の参考**: チームや組織に仕様駆動開発を導入する際の、リポジトリ構成テンプレートとして活用する
- **仕様書の書き方の学習**: 各工程の成果物（プロジェクト憲章・仕様書・設計計画など）の具体的な書き方を参照する
...
このリモートリポジトリは、仕様駆動開発の「**4 つの原則**」と「**7 つの工程**」に沿って構成されています。
...
│       └── errata.md           # 書籍の正誤表
├── examples/                    # サンプルファイル（「7 つの工程」ごとに1ファイル）
...
このリモートリポジトリの`examples/`ディレクトリには、サンプルプロジェクトとして「顧客管理システム」の仕様が含まれています。「7 つの工程」すべての例を体験できます：

- **原則決定工程**：プロジェクト憲章の書き方
- **企画・要件定義工程**：仕様書の書き方と、「4 つの原則」に基づく運用
...
## spec-ai-writer（仕様書自動生成ツール）

`spec-ai-writer/` ディレクトリには、仕様駆動開発を支援するオプションのAIツールが含まれています。LLM APIを使ってインタビュー形式で対話を行い、「7 つの工程」の仕様書を自動生成します（Python 3.9以上・Node.js必要）。

詳しい使い方は [spec-ai-writer/README.md](spec-ai-writer/README.md) を参照してください。
...
レビューでいただいた指摘への回答・補足をまとめた動画も公開しています。あわせてご覧ください。  
