...
- Customer暗号鍵はDB、source code、Git repositoryへ保存しない。applicationは環境設定からcurrent key IDとkey IDから32-byte keyへのmappingを持つkey ringを取得する。productionではAWS Secrets ManagerからECS Taskへ環境変数としてinjectする。testではproduction keyとは異なるtest専用keyを使用する。
